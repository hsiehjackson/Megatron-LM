...
class TestGPTUnifiedModel:
    ...
    def setup_method(self, method):
        ...
        config = GPTConfig(num_layers=2, 
                            hidden_size=12, 
                            num_attention_heads=4, 
                            use_cpu_initialization=True,
                            position_embedding_type="learned_absolute",
                            rotary_percent=1.0,
                            model_decoder_layer_spec=get_gpt_layer_with_transformer_engine_spec(), 
                            vocab_size=100, 
                            max_sequence_length=4)
        self.gpt_model = GPTModel(config=config)
    ...
    def test_set_input_tensor(self):
        config: GPTConfig = self.gpt_model.config
